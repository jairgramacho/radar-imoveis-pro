...
def _stripe_price_ids_por_plano():
    return {
        'pro': (app.config.get('STRIPE_PRICE_PRO') or '').strip(),
        'empresa': (app.config.get('STRIPE_PRICE_EMPRESA') or '').strip(),
    }


def _stripe_checkout_habilitado():
    secret_key = (app.config.get('STRIPE_SECRET_KEY') or '').strip()
...
def assinatura_checkout():
    ...
    try:
        stripe_client.api_key = (app.config.get('STRIPE_SECRET_KEY') or '').strip()
        success_url = f"{app.config['APP_URL']}{url_for('dashboard')}?assinatura=sucesso"
        cancel_url = f"{app.config['APP_URL']}{url_for('dashboard')}?assinatura=cancelada"
        ...
        app.logger.warning('Falha ao criar checkout Stripe: %s', str(e), exc_info=True)
...
def stripe_webhook():
    ...
    try:
        ...
        db.session.rollback()
        app.logger.error('Erro ao processar webhook Stripe: %s', str(e), exc_info=True)
